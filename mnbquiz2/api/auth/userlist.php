...
$time = date('2016-09-30 00:00:00');

//第一部分
//更新今日增量
$updateTodayData = updateTodayData($config);
var_dump($updateTodayData);
error_log('增量添加:'.$updateTodayData);
...
//第二部分
//替换openid
$result = https_post('http://platform.gypserver.com/demo.php');

$jsoninfo = json_decode($result, true);//神神数据包

//var_dump($jsoninfo);

$failCount = 0;
foreach($jsoninfo as $item){
    $response = saveOpenid($item,$config);
    if($response == 0){
        $failCount++;
        error_log('上传openid失败的是：'.$item['openid']);
        echo '上传openid失败的是：'.$item['openid'].'<br>';
    }
}
echo '上传失败总量：'.$failCount.'<br>';
//名士粉丝更新结束
//-------------------------------------------------------------------
//$response = getUserlist($jsoninfo['openid'],$config);
//var_dump($response);
$data = getOpenidUnionid($config);//获取unionid列表
var_dump($data);
...
function updateTodayData($config)
{
    global $time;
...
//    $time = date('Y-m-d 00:00:00');
    ...
    if ($stmt = $mysqli->prepare('INSERT INTO new_mnb_quiz(openid,quiz_id,quiz_answer,created_at) SELECT openid,quiz_id,quiz_answer,created_at FROM mnb_quiz WHERE mnb_quiz.created_at > ?')
    ) {
    ...
    if ($stmt = $mysqli->prepare('INSERT INTO new_mnb_user(openid,mobile,status,type,created_at,utm_source) SELECT openid,mobile,status,type,created_at,utm_source FROM mnb_user WHERE mnb_user.created_at > ?')
    ) {
        $stmt->bind_param('s', $time);
        $stmt->execute();
    ...
    if ($stmt = $mysqli->prepare('INSERT INTO new_win_users(openid,mobile,user_name,address,created_at,type,updated_at) SELECT openid,mobile,user_name,address,created_at,type,updated_at FROM win_users WHERE win_users.created_at > ?')
    ) {
        $stmt->bind_param('s', $time);
        $stmt->execute();
...
//存入数据库
function saveOpenid($user, $config)
{
    $openid = $user['openid'];
    ...
    if ($stmt = $mysqli->prepare('INSERT INTO mingshi_user(openid,unionid) VALUES(?,?)')
    ) {
        $stmt->bind_param('ss', $openid, $unionid);
        $stmt->execute();
...
function updateOpenid($newOpenid, $oldOpenid, $config)
{
    ...
    if ($stmt = $mysqli->prepare('UPDATE new_mnb_user SET openid = ? WHERE openid = ?')
    ) {
    ...
    if ($stmt = $mysqli->prepare('UPDATE new_win_users SET openid = ? WHERE openid = ?')
    ) {
        $stmt->bind_param('ss', $newOpenid, $oldOpenid);
        $stmt->execute();
    ...
    if ($stmt = $mysqli->prepare('UPDATE new_mnb_quiz SET openid = ? WHERE openid = ?')
    ) {
        $stmt->bind_param('ss', $newOpenid, $oldOpenid);
        $stmt->execute();
...
function getOpenidUnionid($config)
{
    ...
    if ($stmt = $mysqli->prepare('SELECT openid FROM new_mnb_user WHERE openid LIKE "oUDu5%"')
    ) {
        $stmt->execute();
...
//    $time = date('Y-m-d 00:00:00');
    ...
    if ($stmt = $mysqli->prepare('SELECT openid,type,created_at,status FROM new_mnb_user WHERE created_at > ?')
    ) {
        $stmt->bind_param('s', $time);
        $stmt->execute();
    ...
    if ($stmt = $mysqli->prepare('SELECT id,openid,quiz_id,quiz_answer,created_at FROM new_mnb_quiz WHERE created_at > ?')
    ) {
        $stmt->bind_param('s', $time);
        $stmt->execute();
    ...
    if ($stmt = $mysqli->prepare('SELECT openid,user_name,address,mobile,created_at FROM new_win_users WHERE created_at > ?')
    ) {
        $stmt->bind_param('s', $time);
        $stmt->execute();
